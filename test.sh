...
DEFAULT_INPUT="test/samples/*.txt"
FORK=${1:-""}
...
if [ "$#" -eq 0 ] || [ "$#" -gt 2 ] || [ "$FORK" = "-h" ]; then
  ...
  echo "./test.sh baseline test/samples/measurements-1.txt"
  echo "./test.sh baseline 'test/samples/measurements-*.txt'"
  exit 1
fi
...
for sample in $(ls $INPUT); do
  echo "Validating calculate_$FORK.rb -- $sample"

  rm -f measurements.txt
  ln -s $sample measurements.txt

  diff --color=always <(ruby calculate_$FORK.rb | ./tocsv.sh) <(./tocsv.sh < ${sample%.txt}.out)
done
